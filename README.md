...
My Library is a personal book catalogue that runs entirely in the browser, with no server, no installation and no build step: opening the main file is enough to start using it. It grew out of a concrete and deliberately narrow need — keeping track of one's own volumes, of where they sit on the shelves, and above all of loans, which are the most common way of losing sight of a book. The application allows entries to be added, edited and deleted, each holding title, author, genre, year, ISBN and shelfmark; it supports searching by title, author or code, filtering by genre and by status, and sorting the results by different criteria; it lets a volume be marked as lent out along with the name of the borrower; and finally it can export the whole catalogue to an interchange file, or read one back in, offering the choice between replacing what is currently held, adding only what is new, or updating the entries already present while leaving the rest untouched.
The project is written in HTML, CSS and JavaScript, the three foundational languages of the web, and is organised into three separate files following the principle of separation of concerns: one defines the structure of the content, one the visual appearance, one the behaviour. This division is not a formality but a practical measure, since it makes it possible to work on the presentation without touching the logic and the other way round, and keeps the code readable months later. The interface builds on the Bootstrap framework for its grid and components, and is customised with a palette drawn from the materials of bookbinding — the paper, the green of the covers, the brass of the shelf labels — with the intention of echoing the physical book in its digital representation.
The architecture rests on a single principle: there is one source of truth, namely the list of books held in memory, and everything that appears on screen is recalculated from it. The statistics, the cards, the entries in the filter menus are not data stored separately but instantaneous projections of that one list. This choice makes structurally impossible the most insidious class of error in applications of this kind, the one in which the stored content and what is displayed drift apart with nothing to signal it. A second decision concerns validation: every piece of incoming data, whether it comes from the entry form, from the browser's memory or from an imported file, passes through a single checkpoint that normalises its format and corrects its anomalies, so that the rest of the program can work without having to verify anything.
The data resides in the browser's local storage, a choice that removes every external dependency but carries a limitation stated openly: it is tied to one device and one browser, and can be lost by clearing browsing data. This is precisely why exporting is not an accessory but an essential function, paired with an import capable of rebuilding the catalogue, merging it with another, or bringing it up to date.
As this was my first project in JavaScript, I chose to proceed indirectly: before writing a single instruction I set out in pseudocode the behaviour of each function, establishing what it should receive, what it should return and how it should react to unforeseen cases, and only afterwards moved on to the actual writing, drawing on the support of artificial intelligence tools for the translation into code and for review. The work on the logic remained on my own, as did the identification of the problems that surfaced during testing, and the pseudocode proved to be not a preparatory stage but the most useful documentation the project has.
