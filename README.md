...
My Library is a personal book catalogue that runs entirely in the browser — no 
server, no installation, no build step. It grew out of a narrow, concrete need: 
keeping track of one's own volumes, of where they sit on the shelves, and above 
all of loans. Entries hold title, author, genre, year, ISBN and shelfmark, and 
can be searched, filtered, sorted, marked as lent out, exported to a file and 
read back in. The code is written in HTML, CSS and JavaScript, split across three 
files so that presentation and logic can be changed independently, with an interface 
built on Bootstrap and a palette drawn from the materials of bookbinding. 
Architecturally it rests on one principle: a single source of truth, the list of 
books in memory, from which everything on screen is recalculated, so that stored 
content and display can never drift apart. As this was my first project in JavaScript,
I set out each function's behaviour in pseudocode before writing any code, using AI 
support for the translation and review while the logic remained my own.
